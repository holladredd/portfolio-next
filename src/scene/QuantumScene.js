    ...
    <div className="fixed inset-0 -z-10 bg-black">
      <Canvas shadows dpr={[1, 2]} gl={{ antialias: true, alpha: true, powerPreference: "high-performance" }}>
        <Suspense fallback={null}>
          <color attach="background" args={[theme === "dark" ? "#000000" : "#0f172a"]} />
          ...
          <EnergyLinks />
          <group>{clusters.map(c => <Cluster key={c.id} {...c} />)}</group>
          <Dredd />
          <DreddAI />
          <OrbitControls enableZoom={false} enablePan={false} rotateSpeed={0.3} makeDefault />
        </Suspense>
      </Canvas>
    </div>
  );
