...
export default function Exhibit({ position, rotation, data, type = "panel" }) {
  ...
  return (
    <group position={position} rotation={rotation}>
      {/* Exhibit Spotlight - The Museum Focus */}
      <spotLight 
        position={[0, 4, 3]} 
      ...
      <Float speed={2} rotationIntensity={0.2} floatIntensity={0.2}>
        <group 
          onClick={() => setExhibit(data)}
          onPointerOver={() => setHovered(true)}
          onPointerOut={() => setHovered(false)}
        >
          {type === "panel" ? (
            /* Project Panel: Polished Carbon */
            <mesh>
              ...
              <lineSegments>
                 ...
                 <lineBasicMaterial color={hovered ? "#38bdf8" : "#334155"} linewidth={2} />
              </lineSegments>
            </mesh>
          ) : (
            /* Skill/Social Node: Surgical Steel Sphere */
            <group>
               <mesh>
          ...
          <Text 
            position={[0, type === "panel" ? -1.8 : -1.2, 0.1]} 
            fontSize={0.25} 
            color="white" 
