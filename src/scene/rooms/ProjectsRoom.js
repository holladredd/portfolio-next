...
export default function ProjectsRoom({ position }) {
  ...
  return (
    <group position={position}>
      ...
      <EntranceDoor position={[0, 0, 19.8]} rotation={[0, Math.PI, 0]} label="BACK TO LOBBY" onClick={() => setRoom("lobby")} />
      
      {/* Gallery Hallway - Left and Right Walls */}
      {projects.map((p, i) => {
        const row = i % 2 === 0 ? -8 : 8;
      ...
      {/* Graphics Annex Portal - At the end of the hall */}
      <EntranceDoor 
        position={[0, 0, -19.8]} 
        label="GRAPHICS ANNEX" 
        onClick={() => setRoom("graphics")} 
      />
    </group>
  );
}
