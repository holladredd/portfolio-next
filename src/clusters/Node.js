...
export default function Node({ id, position, text, parentClusterId }) {
  const meshRef = useRef();
  const materialRef = useRef();
  const { mouse } = useThree();
  const { focusedNode, setFocusedNode, setFocusedCluster, theme } = useStore();
  ...
  const handleNodeClick = (e) => {
    ...
    // Travel ProtocolSync: set cluster focus for camera framing
    if (parentClusterId) setFocusedCluster(parentClusterId);
