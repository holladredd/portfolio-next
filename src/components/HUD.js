...
export default function HUD() {
  ...
  return (
    <>
      {/* System Location Indicator (Make Prev Room Visible) */}
        ...
        {isHUDVisible && (
          ...
          >
            ...
            <div className="space-y-6">
              <h2 className="text-4xl font-anta text-white">{hudData?.name}</h2>
              <div className="h-1 w-20 bg-blue-500" />
              <p className="text-gray-400 leading-relaxed text-lg">
                {hudData?.desc || "Technical specification and development details for this digital exhibit."}
              </p>
              {hudData?.techs && (
                <div className="flex flex-wrap gap-2">
                  {hudData.techs.map((t) => (
                    <span key={t} className="px-3 py-1 bg-white/5 border border-white/10 text-blue-400 text-xs font-mono">
                      {t}
                    </span>
                  ))}
                </div>
              ...
              {hudData?.link && (
                <a 
                  ...
                  className="inline-block px-8 py-3 bg-blue-600 text-white font-anta hover:bg-blue-500 transition-all"
                >
                  VIEW SOURCE_GITHUB
                </a>
              )}
